...
selenium_ignored_tests{action_id="${author="${GITHUB_ACTOR}", os="${RUNNER_OS}"} $(echo ${TEST_REPORT} | awk -F'"' '{ print $2 }')
selenium_total_tests{action_id="${author="${GITHUB_ACTOR}", os="${RUNNER_OS}"} $(echo ${TEST_REPORT} | awk -F'"' '{ print $4 }')
selenium_total_tests_passed{action_id="${author="${GITHUB_ACTOR}", os="${RUNNER_OS}"} $(echo ${TEST_REPORT} | awk -F'"' '{ print $6 }')
selenium_total_tests_failed{action_id="${author="${GITHUB_ACTOR}", os="${RUNNER_OS}"} $(echo ${TEST_REPORT} | awk -F'"' '{ print $8 }')
selenium_total_tests_skipped{action_id="${author="${GITHUB_ACTOR}", os="${RUNNER_OS}"} $(echo ${TEST_REPORT} | awk -F'"' '{ print $10 }')
selenium_suite_execution_duration{action_id="${author="${GITHUB_ACTOR}", os="${RUNNER_OS}"} $(echo ${SUITE} | awk -F'"' '{ print $10 }')
EOF
